...
const MetaDataPage = (props) => {
  ...
  const rid =
    queryParams && queryParams.id
      ...
      : "";
  const inMapping =
    rid !== "" ? mapping.findIndex((m) => m.id === rid) > -1 : false;
  ...
  const backtoSearch = () => {
    if (Array.isArray(backArr) && backArr.length > 0) {
      ...
      history.push({
        pathname: "/",
        search: `keyword=${encodeURI(stateKeyword.trim())}${
          stateKO ? "&ksonly" : ""
        }`,
